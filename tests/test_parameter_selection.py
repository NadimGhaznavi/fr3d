...
class SelectionTests(HistoryFixture, unittest.TestCase):
    ...
    def test_sql_values_match_all_other_columns_and_count_statuses(self):
        ...
        self.add_run(1)
        candidate = self.configuration.candidate(self.baseline, parameter, {'value': 0})
        ...
        self.assertEqual(rows, [
            {'value': -2, 'completed_count': 1, 'gold_count': 1},
            {'value': 0, 'completed_count': 1, 'gold_count': 0},
        ])
    ...
    def test_selection_never_reads_episode_scores_or_builds_reports(self):
        ...
        self.assertEqual(selected, ('game.rewards.closer_to_food', True))
    ...
    def test_exhausted_rewards_log_counts_and_reopen_for_different_gold(self):
        parameter = 'game.rewards.further_from_food'
...
class SelectionLoopTests(HistoryFixture, unittest.IsolatedAsyncioTestCase):
    ...
    async def test_report_is_built_only_for_selected_parameter_and_passed_unchanged(self):
        parameter = 'game.rewards.closer_to_food'
        self.conversation.run.return_value = self.configuration.candidate(self.baseline, parameter, {'value': 0})
