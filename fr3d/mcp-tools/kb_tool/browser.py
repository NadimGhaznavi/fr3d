...
DOCUMENT_ROOT = Path(__file__).resolve().parents[3] / "fr3dnet"
MAX_URL_LENGTH = 256
...
class KbBrowser:
    """Resolve and validate pages within this browser's document root."""

    def __init__(self, document_root: Path = DOCUMENT_ROOT) -> None:
        self.document_root = document_root
        server_log = Path(DEFDIR.SERVER_LOGS / DEFFILE.LLM_SERVER_LOG)
        self.log = MyLog(client_id=MODULE.KB_BROWSER, log_file=server_log, to_console=False)
    ...
    def resolve_url(self, url: str) -> Path:
        ...
        resolved = (self.document_root / relative).resolve(strict=False)
    ...
    def validate_markdown(self, content: str, source: Path) -> None:
    ...
    def load_page(self, url: str = "/") -> str:
        ...
        self.log.info(f"load_page({url})")
        page = self.resolve_url(url)
        content = page.read_text(encoding="utf-8")
        self.validate_markdown(content, page)
        return content
