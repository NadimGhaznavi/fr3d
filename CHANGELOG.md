...
- Default the release script's next feature branch to `feat/maint-` followed by
  the release version with its patch number incremented. An explicit branch name
  remains optional.

## [0.22.15] - 2026-09-09 @ 18:13
...
## [0.22.12] - 2026-09-09 @ 17:25

### Changed

- Decide automatic submissions from parameter selection's availability result.
  Sole remaining scalar values and pairs bypass report construction and the LLM;
  reports no longer recalculate scalar availability to control submission.
...
- Retain and display the latest sample of each prompt type per parameter, so
  initial, summary, and correction requests no longer overwrite one another.
...
- Increased the LLM conversation timeout from 240 to 660 seconds to allow more
  time for response generation and invalid or duplicate proposal corrections.
